...
def hello_api(request):
    ...
    greeting = f"Hello, {visitor_name}!, the temperature is {temperature} degrees Celsius in {city}" if temperature else f"Hello, {visitor_name}! The temperature data is unavailable for {city}"
    response_data = {
        'client_ip': client_ip,
        'location': city,
        'greeting': greeting
